...
#### 4. إصلاحات لوحة تحكم الشركاء والتشغيل الخارجي (`vendor-dashboard.html`)
* **إصلاح فلتر تبويب "رحلات الغد" و "رحلات اليوم"**:
  * تحسين دالة `parseDateToYMD` لدعم كافة تنسيقات التواريخ (`YYYY-MM-DD`, `DD-MM-YYYY`, `YYYY/MM/DD`, `DD/MM/YYYY`, وتوحيد الأرقام العربية والإنجليزية).
  * إضافة دالة `extractCleanTripInfo` لاستخراج التاريخ والوقت بدقة ومطابقتها مع تاريخ اليوم وتاريخ الغد.
* **تنظيف رسالة الواتساب المرسلة للسائق (أمر الشغل)**:
  * تصفية الملاحظات لحذف كافة وسوم وبيانات النظام الداخلية مثل `[sheet_row:...]`, `[google_res_id:...]`, `حجز موديتور: ...` وإرسال الملاحظة الفعلية فقط للعميل أو كتابة `لا توجد`.
  * إدراج التاريخ والوقت الفعلي للرحلة داخل رسالة السائق بدقة.
* **تصحيح رقم هاتف الكابتن في رسالة العميل**:
  * تصحيح الرقم المرسل للعميل ليكون هاتف الكابتن الحقيقي المسجل في النموذج `${drvPhone}` بدلاً من الرقم الثابت.
